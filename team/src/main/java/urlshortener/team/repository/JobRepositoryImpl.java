...
public class JobRepositoryImpl implements JobRepository {
    ...
    private static final RowMapper<Job> rowMapper = (rs, rowNum) -> {
        ...
        URI uriresult = null;
        try {
            if(rs.getString("uriresult")!=null){
                uriresult = new URI(rs.getString("uriresult"));
            }
        } catch (URISyntaxException e) {
            e.printStackTrace();
        }

        return new Job(rs.getString("hash"), rs.getInt("converted"), rs.getInt("total"), uriresult, result);
    };
    ...
    public Job save(Job j1) {
        try {
            ...
            if (j1.getUriResult() != null) {
                uriResult = j1.getUriResult().toString();
            }
            jdbc.update("INSERT INTO job VALUES (?,?,?,?,?)",
                    j1.getHash(), j1.getConverted(), j1.getTotal(), uriResult, blob);
    ...
    public void update(Job j1) {
        try {
            ...
            jdbc.update(
                    "update job set hash=?, converted=?, total=?, uriresult=?, result=? where hash=?",
                    j1.getHash(), j1.getConverted(), j1.getTotal(),
                    uriResult, blob, j1.getHash());
        } catch (Exception e) {
            e.printStackTrace();
